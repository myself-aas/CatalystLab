...
export const UserDashboardPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 pb-20">
      ...
      {/* Top Header Section */}
      <section className="border-b border-slate-800 bg-slate-900/40 px-4 py-10 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-6xl">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center gap-3">
              <Link
                to="/"
                className="flex items-center gap-2 rounded-xl bg-cyan-500 px-4 py-2.5 text-xs font-bold text-slate-950 transition-all hover:bg-cyan-400 shadow-lg shadow-cyan-500/20"
              >
          ...
          <div className="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-4">
            <div className="rounded-xl border border-slate-800 bg-slate-950/80 p-4">
              <div className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Total Audits</div>
              <div className="mt-1 text-2xl font-black text-white font-mono">{totalAudits}</div>
              <div className="text-[10px] text-slate-500 mt-1">Saved in cloud</div>
            </div>

            <div className="rounded-xl border border-slate-800 bg-slate-950/80 p-4">
              <div className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Unique Domains</div>
              <div className="mt-1 text-2xl font-black text-cyan-400 font-mono">{uniqueDomains}</div>
              <div className="text-[10px] text-slate-500 mt-1">Monitored endpoints</div>
            </div>

            <div className="rounded-xl border border-slate-800 bg-slate-950/80 p-4">
              <div className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Latest Activity</div>
              <div className="mt-1 text-lg font-black text-emerald-400 font-mono truncate">{latestAuditDate}</div>
              <div className="text-[10px] text-slate-500 mt-1">Most recent scan</div>
            </div>

            <div className="rounded-xl border border-slate-800 bg-slate-950/80 p-4">
              <div className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Account Tier</div>
              <div className="mt-1 text-lg font-black text-indigo-400 font-mono">Pro Telemetry</div>
            ...
            {filteredReports.map((report) => {
              ...
              return (
                <div
                  ...
                  className="group relative flex flex-col justify-between rounded-2xl border border-slate-800 bg-slate-900/50 p-5 transition-all hover:border-cyan-500/50 hover:bg-slate-900 hover:shadow-2xl cursor-pointer"
                >
                  <div>
                    {/* Card Header */}
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <div className="flex items-center gap-2.5">
                        <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 text-base">
                          {meta.icon}
                        </span>
                        <div>
                          <h3 className="text-sm font-bold text-white group-hover:text-cyan-400 transition-colors truncate max-w-[170px]">
                            {domain}
                          </h3>
                  ...
                  <div className="flex items-center justify-between border-t border-slate-800/80 pt-3 mt-2">
                    ...
                    <span className="flex items-center gap-1 text-xs font-semibold text-cyan-400 group-hover:translate-x-0.5 transition-transform">
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <span className="rounded bg-slate-800 px-2 py-0.5 text-[11px] text-cyan-400 font-mono">
                            {meta.name}
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-3 text-right">
                          <div className="flex items-center justify-end gap-2" onClick={(e) => e.stopPropagation()}>
                            ...
                            <button
                              ...
                              title="Copy Link"
                            >
                              {copiedId === (report.id || domainSlug) ? <Check className="h-3.5 w-3.5 text-emerald-400" /> : <Share2 className="h-3.5 w-3.5" />}
                            </button>
                            <button
                              onClick={(e) => handleDelete(report.id!, e)}
