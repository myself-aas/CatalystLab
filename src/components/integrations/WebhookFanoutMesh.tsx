...
export const WebhookFanoutMesh: React.FC = () => {
  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-slate-950 text-white p-6 sm:p-10 shadow-2xl relative overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4 border-b border-slate-800/80 pb-6">
          ...
          <button
            ...
            disabled={isFiring}
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 text-slate-950 text-xs sm:text-sm font-mono font-bold transition-all shadow-lg shadow-cyan-500/20 active:scale-95 cursor-pointer disabled:opacity-50"
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          ...
          <div className="lg:col-span-5 space-y-3">
              ...
              return (
                <div
                  ...
                  className={`p-4 rounded-2xl border transition-all cursor-pointer ${
                    isSelected
                      ? 'bg-blue-600/20 border-cyan-500 shadow-lg ring-1 ring-cyan-500/50 text-white'
                      : 'bg-slate-900/60 border-slate-800 hover:bg-slate-900 text-slate-300'
                  }`}
                >
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-2 font-bold font-sans text-sm text-white">
                      <Webhook className={`h-4 w-4 ${isSelected ? 'text-cyan-400' : 'text-slate-500'}`} />
                      <span>{dest.name}</span>
                    </div>
            ...
            {/* Micro Details Box */}
            <div className="p-4 rounded-2xl bg-slate-900/40 border border-slate-800 text-xs font-mono text-slate-400 space-y-1.5">
