    ...
    <div className="rounded-2xl border border-black/30 bg-white p-6 shadow-xl space-y-6 text-black">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-black/25 pb-5">
        ...
        <div className="flex items-center gap-2 text-xs font-mono">
          ...
          <span className="text-xl font-black text-black bg-black/25 px-3 py-1 rounded-xl border border-black/40">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
        {dimensions.map((dim) => {
          ...
          return (
            <div
              key={dim.name}
              className="rounded-xl border border-black/30 bg-slate-50 p-4 transition-all hover:border-black/60 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400"
            >
              <div className="flex items-center justify-between gap-2 mb-3">
                ...
                <span className="font-mono text-sm font-black text-black">{dim.score}/100</span>
              </div>

              <div className="text-xs font-bold text-black truncate">{dim.name}</div>

              {/* Mini progress bar */}
              <div className="h-1.5 w-full rounded-full bg-slate-200 overflow-hidden mt-2.5 border border-black/20">
                <div
