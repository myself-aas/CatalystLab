...
header {
  ...
  return (
    <div className="rounded-2xl border border-black/30 bg-white p-6 shadow-xl space-y-6 text-black">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-black/25 pb-5">
        <div>
          <div className="flex items-center gap-2">
            <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-black/25 text-slate-500 border border-black/40">
        ...
        <div className="flex items-center gap-3">
          <div className="text-right">
            <div className="text-xs font-semibold text-slate-500">Security Index</div>
            <div className="text-xl font-black text-emerald-400 font-mono">{score}/100</div>
          </div>
          <div className={`flex h-12 w-12 items-center justify-center rounded-xl border ${
            score >= 80 ? 'border-emerald-500/30 bg-emerald-500/15 text-emerald-400' : 'border-black/40 bg-black/25 text-slate-500'
          }`}>
      ...
      {/* Compliance Meter */}
      <div className="rounded-xl border border-black/30 bg-white p-4">
        ...
        <div className="h-2.5 w-full rounded-full bg-white overflow-hidden border border-black/30">
          <div
            className={`h-full transition-all ${
              compliancePct >= 80 ? 'bg-emerald-400' : compliancePct >= 50 ? 'bg-black' : 'bg-rose-500'
            }`}
      ...
      {/* Remediation Snippet */}
      <div className="rounded-xl border border-black/30 bg-white p-4 space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center rounded-lg bg-white p-0.5 border border-black/30 text-[11px]">
              <button
                onClick={() => setSnippetFormat('vercel')}
                className={`rounded px-2.5 py-1 font-semibold transition-colors ${snippetFormat === 'vercel' ? 'bg-black text-white' : 'text-slate-500 hover:text-black'}`}
              >
                vercel.json
              </button>
              <button
                onClick={() => setSnippetFormat('nginx')}
                className={`rounded px-2.5 py-1 font-semibold transition-colors ${snippetFormat === 'nginx' ? 'bg-black text-white' : 'text-slate-500 hover:text-black'}`}
              >
                nginx.conf
              </button>
              <button
                onClick={() => setSnippetFormat('caddy')}
                className={`rounded px-2.5 py-1 font-semibold transition-colors ${snippetFormat === 'caddy' ? 'bg-black text-white' : 'text-slate-500 hover:text-black'}`}
              >
                Caddyfile
              </button>
            </div>
            ...
            <button
              onClick={handleCopy}
              className="flex items-center gap-1.5 rounded-lg border border-black/40 bg-black/25 px-3 py-1 text-xs font-semibold text-black hover:bg-black/40 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400"
            >
              {copiedSnippet ? <Check className="h-3.5 w-3.5 text-emerald-400" /> : <Copy className="h-3.5 w-3.5 text-slate-500" />}
              <span>{copiedSnippet ? 'Copied' : 'Copy'}</span>
            </button>
          </div>
        </div>

        <pre className="overflow-x-auto rounded-lg bg-white p-3.5 font-mono text-[11px] text-slate-500 border border-black/30">
