    ...
    <div className="rounded-2xl border border-black/30 bg-white p-6 shadow-xl space-y-6 text-black">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-black/25 pb-5">
        ...
        <div className="flex items-center gap-3">
          <div className="text-right">
            <div className="text-xs font-semibold text-slate-500">Global Average</div>
            <div className="text-xl font-black text-slate-500 font-mono">~{globalAverageMs} ms</div>
          </div>
          <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-black/25 text-slate-500 border border-black/40">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between text-xs text-slate-500 mb-1">
          ...
          <span className="font-mono text-[11px] text-slate-500">{infrastructure}</span>
        </div>
        ...
        <div className="space-y-2.5">
          {pops.map((pop) => {
            ...
            return (
              <div key={pop.location} className="rounded-xl border border-black/30 bg-white p-3">
                ...
                <div className="h-2 w-full rounded-full bg-white overflow-hidden border border-black/20">
                  <div
                    className={`h-full rounded-full transition-all duration-700 ${
                      isFast ? 'bg-emerald-400' : isModerate ? 'bg-slate-500' : 'bg-rose-500'
                    }`}
      ...
      <div className="rounded-xl border border-black/30 bg-white p-4 space-y-3">
        <div className="flex items-center justify-between text-xs font-bold text-black">
          ...
          <span className="font-mono text-slate-500">Total: ~{totalRoundTrip}ms</span>
        </div>

        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
          <div className="rounded-lg bg-white p-2.5 border border-black/30">
            <div className="text-[10px] text-slate-500 uppercase font-mono">1. DNS Resolution</div>
            <div className="text-sm font-bold text-slate-500 font-mono mt-0.5">{dnsTime} ms</div>
          </div>
