      ...
      <motion.div
        ...
        className={`relative flex items-center h-9 rounded-full transition-colors overflow-visible ${
          isOpen
            ...
            : 'bg-[var(--app-background)]/80 border border-[var(--border-subtle)] text-foreground-muted hover:text-foreground hover:bg-[var(--bg-surface)] hover:border-accent/40 shadow-linear-card'
        }`}
      >
          ...
          <div className="flex items-center w-full pr-1.5 overflow-hidden">
            ...
            {query ? (
              <button
                ...
                className="p-1 rounded-md hover:bg-[var(--bg-panel-hover)] transition-colors shrink-0 text-foreground-muted hover:text-foreground"
                title="Clear input"
              >
                <X className="size-3" />
              </button>
              ...
              <button
                ...
                className="px-1.5 py-0.5 rounded-md text-[10px] font-mono shrink-0 transition-colors bg-[var(--bg-surface)] text-foreground-muted hover:text-foreground"
                title="Close search"
              >
                Esc
              </button>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            <div className="flex items-center justify-between px-3.5 py-2.5 border-b border-[var(--border-subtle)] bg-[var(--bg-surface)] text-[10px] uppercase tracking-wider text-foreground-muted">
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-start gap-2.5 px-3 py-2.5 rounded-xl cursor-pointer transition-all duration-150 ${
                        isSelected
                          ? 'bg-[var(--bg-panel-hover)] text-foreground shadow-2xs'
                          : 'hover:bg-[var(--bg-surface)] text-foreground-muted'
                      }`}
                    >
            ...
            {!query && (
              <div className="px-3 py-2 border-t border-[var(--border-subtle)] bg-[var(--bg-surface)] flex flex-wrap items-center gap-1.5 text-[10px] text-foreground-muted">
                <span>Trending:</span>
                <button
                  type="button"
                  onClick={() => handleSelect(STATIC_SUGGESTIONS[0])}
                  className="px-2 py-0.5 rounded-md hover:bg-[var(--bg-panel-hover)] text-foreground font-medium cursor-pointer transition-colors"
                >
                  Health
                </button>
                <span>•</span>
