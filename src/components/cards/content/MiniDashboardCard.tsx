...
export const MiniDashboardCard: React.FC<MiniDashboardCardProps> = ({
  ...
  return (
    <Card
      ...
      hue={hue}
      lift={false}
      className={`p-4 sm:p-5 flex flex-col justify-between border border-border bg-background shadow-sm hover:border-border hover:shadow-md transition-all ${className || ''}`}
    >
      <div>
        ...
        <div className="flex items-center justify-between gap-2 pb-3 mb-3 border-b border-border">
          <div className="flex items-center gap-2 min-w-0">
            <div className="w-7 h-7 rounded-lg bg-muted border border-border flex items-center justify-center text-indigo-600 shrink-0 shadow-sm">
              {icon || <Activity className="w-4 h-4" />}
            </div>
            <CardTitle
              as="h4"
              className="text-xs sm:text-sm font-bold text-foreground font-sans tracking-tight truncate"
            >
              {title}
            </CardTitle>
          </div>
        ...
        <div className="my-3">
          <div className="text-2xl sm:text-3xl font-black font-mono text-foreground tracking-tight metric-tabular">
            {mainValue}
          </div>
          ...
          {subtitle && (
            <p className="text-[11px] font-sans font-medium text-muted-foreground mt-1.5 leading-relaxed">
              {subtitle}
            </p>
          )}
        </div>
