    ...
    <div className="w-full max-w-5xl mx-auto p-4 md:p-6 bg-muted text-foreground rounded-xl">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="ds-muted text-sm mt-1">Connect repositories for automated CI/CD audits.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center p-12 bg-background border border-border rounded-2xl shadow-sm">
          <GitBranch className="w-12 h-12 ds-muted mx-auto mb-4" />
          <h3 className="text-lg font-bold ds-muted">No Repositories Connected</h3>
          <p className="ds-muted mt-2 max-w-md mx-auto">Configure your first GitHub webhook to trigger automated audits on every pull request.</p>
        </div>
      ) : (
            ...
            <motion.div key={repo.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center justify-between p-5 bg-background border border-border rounded-2xl shadow-sm">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-full bg-accent flex items-center justify-center">
                  <GitBranch className="w-5 h-5 ds-muted" />
                </div>
                <div>
