    ...
    <div className="w-full max-w-5xl mx-auto p-4 md:p-6 bg-muted text-foreground rounded-xl">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Manage programmatic access to CatalystLab endpoints.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center p-12 bg-background border border-border rounded-2xl shadow-sm">
          ...
          <h3 className="text-lg font-bold text-muted-foreground">No API Keys</h3>
          <p className="text-muted-foreground mt-2 max-w-md mx-auto">Generate a secure token to authenticate your backend servers or CI/CD pipelines.</p>
        </div>
      ) : (
            ...
            <tbody className="divide-y divide-border">
              {keys.map(key => (
                <tr key={key.id} className="hover:bg-muted/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-muted-foreground">{key.lastUsedAt ? new Date(key.lastUsedAt).toLocaleDateString() : 'Never'}</td>
                  <td className="px-6 py-4 text-right space-x-2">
                    <button className="p-1.5 text-muted-foreground hover:text-muted-foreground bg-muted hover:bg-accent rounded-md transition-colors"><Copy className="w-4 h-4" /></button>
