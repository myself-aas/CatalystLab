        ...
        <div className="lg:grid lg:grid-cols-12 gap-8 mt-12">
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors border ${
                    activeCategory === cat
                      ...
                      : 'bg-[var(--bg-surface)] text-muted-foreground border-foreground/10 hover:border-foreground/30 hover:text-foreground'
                  }`}
                >
                  {cat}
                </button>
            ...
            <div className="flex flex-col gap-3">
              {filteredFaqs.map((faq, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="border border-foreground/10 bg-surface rounded-2xl overflow-hidden"
                  >
                    <button
                      onClick={() => setOpenIndex(isOpen ? null : idx)}
                      className="w-full flex items-center justify-between p-4 sm:p-5 text-left"
                    >
                      <span className="framer-card-title text-foreground">
                        {faq.q}
                      </span>
                      <div className="shrink-0 ml-4 flex items-center justify-center size-6 rounded-full bg-[var(--bg-surface)] text-foreground/50">
                        {isOpen ? <Minus className="size-3.5 text-foreground" /> : <Plus className="size-3.5" />}
                      </div>
                    </button>
                    <AnimatePresence>
                      {isOpen && (
                        ...
                        >
                          <div className="px-4 sm:px-5 pb-5 framer-body-text border-t border-foreground/5 pt-4">
                            {faq.a}
                          </div>
                        </motion.div>
                      )}
          ...
          <div className="lg:col-span-7 mt-8 lg:mt-0">
            <div
              ...
              className="bg-background border border-foreground/10 rounded-2xl md:rounded-3xl overflow-hidden h-full flex flex-col shadow-2xl relative group transition-[transform,border-color] duration-200 ease-out hover:-translate-y-[2px] hover:border-foreground/25"
            >
              <div
...
<span className="text-foreground/40">$</span> curl -sSL https://api.catalystlab.tech/v2/audit \
    ...
    -d <span className="text-emerald-400">'{'{"domain": "target.io", "engines": "all", "auto_patch": true}'}'</span>
              ...
              {/* Terminal Footer */}
              <div className="p-4 border-t border-foreground/10 bg-surface flex justify-end relative z-10">
                <CopyButton
                  text={CURL_SNIPPET}
                  variant="terminal"
                  label="Copy Command"
