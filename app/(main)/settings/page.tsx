...
export default function SettingsPage() {
  ...
  const handleSaveKeys = () => {
    ...
    setTimeout(() => {
      setIsSaving(false);
      setSaveMessage({ text: 'API keys updated successfully', type: 'success' });
  ...
  const sourceDetails = [
    { id: 'ss', name: 'Semantic Scholar', desc: '200M+ papers, all disciplines', color: 'var(--src-ss)' },
    { id: 'oa', name: 'OpenAlex', desc: '250M+ works, fully open index', color: 'var(--src-oa)' },
    { id: 'arxiv', name: 'arXiv', desc: '2.3M+ preprints in CS, Physics, Math', color: 'var(--src-arxiv)' },
    { id: 'pm', name: 'PubMed', desc: '36M+ biomedical citations', color: 'var(--src-pm)' },
    { id: 'core', name: 'CORE', desc: 'World\'s largest open access collection', color: 'var(--src-core)' },
    { id: 'cr', name: 'Crossref', desc: '150M+ scholarly works, DOI metadata', color: 'var(--src-cr)' },
    { id: 'epmc', name: 'Europe PMC', desc: '42M+ abstracts, life sciences focus', color: 'var(--src-epmc)' },
    { id: 'doaj', name: 'DOAJ', desc: '9M+ articles from open access journals', color: 'var(--src-doaj)' },
    { id: 'upw', name: 'Unpaywall', desc: 'PDF enrichment pass for DOIs', color: 'var(--src-upw)' },
  ];
      ...
      <main className="flex-1 md:ml-[220px] pb-20 md:pb-0">
        ...
        <div className="max-w-5xl mx-auto p-6 md:p-10">
          <header className="mb-12">
            <h2 className="text-[28px] font-semibold text-[var(--text-primary)] tracking-tight mb-2">Settings</h2>
            <p className="text-[14px] text-[var(--text-secondary)]">Configure your research environment and API integrations.</p>
          </header>
            ...
            <aside className="w-full lg:w-56 shrink-0">
              <nav className="flex lg:flex-col gap-1 overflow-x-auto lg:overflow-x-visible pb-4 lg:pb-0 scrollbar-hide">
                {tabs.map(tab => (
                  ...
                  >
                    <tab.icon className={cn("w-4 h-4", activeTab === tab.id ? "text-[var(--accent)]" : "text-[var(--text-tertiary)]")} />
                    {tab.label}
                  </button>
                ))}
              </nav>
              ...
              <div className="min-w-full">
                ...
                {activeTab === 'keys' && (
                  ...
                  >
                    <section className="space-y-6">
                      <div className="space-y-1">
                        <h3 className="text-[18px] font-semibold text-[var(--text-primary)]">API Keys</h3>
                        <p className="text-[13px] text-[var(--text-secondary)]">Your keys are stored locally in your browser and never sent to our servers.</p>
                      </div>
                      ...
                      <div className="space-y-8">
                        ...
                        <div className="space-y-4">
                          <div className="flex items-center justify-between">
                            <label className="text-[11px] font-bold text-[var(--text-tertiary)] uppercase tracking-widest">Gemini API Key</label>
                            <a href="https://aistudio.google.com/app/apikey" target="_blank" className="text-[11px] font-bold text-[var(--accent)] hover:underline flex items-center gap-1">
                        ...
                        <div className="space-y-4">
                          <div className="flex items-center justify-between">
                            <label className="text-[11px] font-bold text-[var(--text-tertiary)] uppercase tracking-widest">CORE API Key (Optional)</label>
                            <a href="https://core.ac.uk/services/api" target="_blank" className="text-[11px] font-bold text-[var(--accent)] hover:underline flex items-center gap-1">
                              Get free key <ExternalLink className="w-3 h-3" />
                            </a>
                          </div>
                          <div className="relative">
                            ...
                            <button 
                              ...
                              className="absolute right-4 top-1/2 -translate-y-1/2 text-[var(--text-tertiary)] hover:text-[var(--text-secondary)]"
                            >
                              {showCoreKey ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                            </button>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="p-4 bg-[var(--bg-sunken)] border border-[var(--border-faint)] rounded-[var(--r-lg)] flex items-start gap-3">
                        ...
                        <p className="text-[12px] text-[var(--text-secondary)] leading-relaxed">
                          The following 7 sources require no API key: Semantic Scholar, OpenAlex, arXiv, PubMed, Crossref, Europe PMC, DOAJ, and Unpaywall.
                        </p>
                      </div>
                    </section>
                        ...
                        {saveMessage && (
                          <motion.div 
                            ...
                            className={cn(
                              ...
                              saveMessage.type === 'success' ? "text-[var(--emerald)]" : "text-[var(--rose)]"
                            )}
                          >
                ...
                {activeTab === 'sources' && (
                  ...
                  >
                    <section className="space-y-6">
                      <div className="space-y-1">
                        <h3 className="text-[18px] font-semibold text-[var(--text-primary)]">Research Sources</h3>
                        <p className="text-[13px] text-[var(--text-secondary)]">Toggle which academic databases to include in your searches.</p>
                      </div>
                          ...
                          >
                            <div className="flex items-center gap-4">
                              ...
                              <div className="space-y-0.5">
                                <div className="text-[14px] font-medium text-[var(--text-primary)]">{src.name}</div>
                                <div className="text-[11px] text-[var(--text-tertiary)]">{src.desc}</div>
                              </div>
                            </div>
                            <button 
                              ...
                              className={cn(
                                ...
                                sources[src.id as keyof typeof sources] ? "bg-[var(--accent)]" : "bg-[var(--bg-sunken)] border border-[var(--border-default)]"
                              )}
                            >
                              <div className={cn(
                                "absolute top-1 w-3 h-3 bg-white rounded-full transition-all duration-300",
                                sources[src.id as keyof typeof sources] ? "right-1" : "left-1"
                              )} />
                            </button>
                          </div>
                        ))}
                ...
                {activeTab === 'account' && (
                  ...
                  >
                    <section className="space-y-6">
                      <div className="space-y-1">
                        <h3 className="text-[18px] font-semibold text-[var(--text-primary)]">Account Management</h3>
                        <p className="text-[13px] text-[var(--text-secondary)]">Manage your account security and data.</p>
                      </div>

                      <div className="space-y-4">
                        <div className="p-6 bg-[var(--bg-elevated)] border border-[var(--border-subtle)] rounded-[var(--r-xl)] flex items-center justify-between">
                          <div className="space-y-1">
                            <div className="text-[14px] font-medium text-[var(--text-primary)]">Export Research Data</div>
                            <div className="text-[12px] text-[var(--text-tertiary)]">Download all your sessions and saved papers in JSON format.</div>
                          </div>
                        ...
                        <div className="p-6 bg-[var(--rose-muted)] border border-[var(--rose)]/20 rounded-[var(--r-xl)] flex items-center justify-between">
                          <div className="space-y-1">
                            <div className="text-[14px] font-medium text-[var(--rose)]">Delete All Data</div>
                            <div className="text-[12px] text-[var(--rose)]/60">Permanently remove all your research history. This cannot be undone.</div>
                          </div>
