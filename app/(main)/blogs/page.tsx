...
import { PenTool, Eye, Send, Loader2, Search, Save, User, Bold, Italic, List, Link as LinkIcon, FileText, Heading1, Heading2, Quote, ListOrdered, Code, Braces, Image, Table, Minus, Strikethrough } from 'lucide-react';
...
export default function BlogsPage() {
  ...
  useEffect(() => {
    if (!loading && user) {
      router.replace('/dashboard?tab=blogs');
    }
  }, [user, loading, router]);
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 sm:p-6 space-y-8">
      ...
      {!user && (
        <div className="relative overflow-hidden bg-gradient-to-r from-[#CFFFDC]/40 to-[#68BA7F]/15 rounded-[2rem] p-6 border border-[#68BA7F]/20 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm">
          <div className="space-y-1 text-left">
            <h3 className="text-lg font-bold text-[#253D2C]">Publish Your Research & Insights</h3>
            <p className="text-xs text-[#2E6F40]/80 leading-relaxed max-w-xl">
              Join CatalystLab's vibrant scholastic circle to publish templates, draft scientific articles using markdown, comment on breakthroughs, and save your favourite posts.
            </p>
          </div>
          <Link href="/login" className="px-5 py-3 rounded-xl bg-[#2E6F40] hover:bg-[#253D2C] text-white text-xs font-bold font-mono transition-all uppercase tracking-wider shrink-0 shadow-sm hover:scale-[1.02]">
            Join CatalystLab
          </Link>
        </div>
      )}
