...
import { TOKEN_CONFIG, GEMINI_FREE_TIER_LIMITS } from './tokens-config';
import { FieldValue } from 'firebase-admin/firestore';

export async function verifyAndDeductTokens(req: NextRequest, cost: number) {
  try {
    ...
    const result = await adminDb.runTransaction(async (transaction) => {
      ...
      if (now.toDateString() !== lastReset.toDateString()) {
        ...
        transaction.update(userRef, {
          tokens_remaining: tokensRemaining - cost,
      ...
      if (now.toDateString() !== statsLastReset.toDateString()) {
        ...
        transaction.update(statsRef, {
          ...
          daily_tokens: FieldValue.increment(cost)
        });
      }

      return { success: true, uid, tokensRemaining: tokensRemaining - cost };
    });

    return result;
  } catch (error: any) {
    console.error('vVerifyAndDeductTokens Error:', error);
    return { error: error.message || 'Token verification failed', status: error.message === 'Insufficient tokens' ? 402 : 500 };
  }
}
