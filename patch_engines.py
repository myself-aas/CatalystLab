...
engines = glob.glob("python-engines/*.py")

for eng in engines:
    basename = os.path.basename(eng)
    with open(eng, 'r') as f:
        content = f.read()
    
    if "---CATALYST_METRICS---" in content:
        continue
...
try:
    ...
    elif "{basename}" == "website_health.py":
    ...
    elif "{basename}" == "eco_carbon_audit.py":
    ...
    elif "{basename}" == "compliance_risk_audit.py":
    ...
    elif "{basename}" == "platform_migration_audit.py":
    ...
    elif "{basename}" == "llmo_optimizer.py":
