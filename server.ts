...
async function startServer() {
  ...
  const serverConnectedRepos = new Map<string, any>([
    [
      'gh_repo_default_01',
      {
        ...
        defaultBranch: 'main',
        webhookSecret: 'cat_whsec_9f83a12bc4e5',
  ...
  const handleGithubWebhook = async (req: Request, res: Response) => {
    ...
    const repoIdParam = (req.query?.repoId as string) || req.body?.repoId || 'gh_repo_default_01';
    
    const rawBody = req.body || {};
    const repo = serverConnectedRepos.get(repoIdParam) || {
      id: repoIdParam,
      name: rawBody.repository?.full_name || 'connected/repository',
      defaultBranch: rawBody.repository?.default_branch || 'main',
      ownerId: 'usr_default',
      webhookSecret: ''
    };
  ...
  app.get('/api/v1/integrations/github/repos', (req: Request, res: Response) => {
    const repos = Array.from(serverConnectedRepos.values());
    res.json({
      success: true,
      count: repos.length,
      repos
  ...
  app.post('/api/v1/integrations/github/repos/:id/test-payload', async (req: Request, res: Response) => {
    const { id } = req.params;
