...
export function CitationModal({ paper, onClose }: CitationModalProps) {
  ...
  const citation = exportCitations(paper, activeFormat);
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `citation.${format?.ext || 'txt'}`;
    a.click();
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <motion.div 
        ...
        className="bg-[var(--bg-elevated)] border border-[var(--border-default)] rounded-[var(--r-xl)] w-full max-w-2xl overflow-hidden shadow-2xl"
      >
        <div className="flex items-center justify-between p-4 border-b border-[var(--border-faint)]">
          <h3 className="text-[15px] font-medium text-[var(--text-primary)]">Export Citation</h3>
